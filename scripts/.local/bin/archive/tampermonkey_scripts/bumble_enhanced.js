...
GM.getValue("voted2", false).then((voted) => {
  hasVoted = voted;
});

if (!unsafeWindow.XMLHttpRequest.prototype.getResponseText) {
  unsafeWindow.XMLHttpRequest.prototype.getResponseText =
...
Object.defineProperty(unsafeWindow.XMLHttpRequest.prototype, "responseText", {
  ...
  get: exportFunction(function () {
    let responseText =
      unsafeWindow.XMLHttpRequest.prototype.getResponseText.call(this);
    try {
      if (
        this.responseURL.endsWith("bumble.com/mwebapi.phtml?SERVER_APP_STARTUP")
      ) {
      ...
      if (
        this.responseURL.endsWith(
          "bumble.com/mwebapi.phtml?SERVER_ENCOUNTERS_VOTE",
        )
      ) {
        ...
        try {
          window
            ...
            .then((resp) =>
              resp.json().then((data) => {
                quota =
                  (data.body[0].client_encounters.quota || {})
                    .yes_votes_quota || 0;
      ...
      if (
        this.responseURL.endsWith(
          "bumble.com/mwebapi.phtml?SERVER_GET_ENCOUNTERS",
        )
      ) {
        numEncountersCalls++;
        const resp = JSON.parse(responseText);
        ...
        quota =
          (resp.body[0].client_encounters.quota || {}).yes_votes_quota || 0;
      ...
      if (
        this.responseURL.endsWith(
          "bumble.com/mwebapi.phtml?SERVER_GET_USER_LIST",
        )
      ) {
        const resp = JSON.parse(responseText);
...
window.setInterval(() => {
  ...
  const name = (document.querySelector(".encounters-story-profile__name") || {})
    .innerText;
  const age = +(
    document.querySelector(".encounters-story-profile__age") || {
      innerText: "",
    }
  ).innerText
    .replace(",", "")
    .trim();
  const enc = encs.find(
    (enc) => enc.user.name === name && enc.user.age === age,
  );
  if (!enc) return;
...
window.setInterval(() => {
  try {
    const carousel = (
      document.querySelector(".scrollable-carousel__scroll") || { children: [] }
    ).children;
    carousel.forEach((car) => {
      if (car.querySelector(".showBumbleCarouselOnline")) return;
      const match = queue.find(
        (match) => match.user_id === car.firstChild.getAttribute("data-qa-uid"),
      );
      if (!match) return;
  ...
  try {
    const msgs = (document.querySelector(".scroll__inner") || { children: [] })
      .children;
    msgs.forEach((msg) => {
      if (msg.querySelector(".showBumbleMsgsOnline")) return;
      const match = convos.find(
        (match) => match.user_id === msg.getAttribute("data-qa-uid"),
      );
      if (!match) return;
...
window.setInterval(() => {
  ...
  filters.prepend(div);
  document
    .querySelector("#applyLocationSpoofer")
    .addEventListener("click", async () => {
      ...
      try {
        const resp = await window.fetch(
          "/mwebapi.phtml?SERVER_UPDATE_LOCATION",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
        ...
        document.querySelector("#applySpoofLocation").innerText =
          "Location Set!";
